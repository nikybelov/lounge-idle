...
import { guestTraffic, expansionIncomeBonus } from './appeal'
...
export function loungeIncomePerSec(state: GameState): number {
  ...
  return sum * (state.loungeIncomeMult || 1) * guestTraffic(state)
}
...
export function loungeClickPower(state: GameState): number {
  ...
  return click * (state.loungeClickMult || 1) * guestTraffic(state)
}
